                  ...
                  <section class="text-gray-600 body-font relative">
                    <div class="container px-5 py-24 mx-auto">
                      <div class="flex flex-col text-center w-full mb-12">
                        <h1 class="sm:text-3xl text-2xl font-medium title-font mb-4 text-gray-900">Edit Job</h1>
                      </div>
                      ...
                      @endforeach
                      <form action="{{route('job.update', $singleJob->slug)}}" method="post">
                      @csrf
                      @method('PUT')
                        <div class="flex flex-wrap -m-2">
                          <div class="p-2 w-1/2">
                            <div class="relative">
                              ...
                              <input type="text" id="title" name="title" value="{{$singleJob->title}}" class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out" required/>
                            </div>
                          </div>
                          <div class="p-2 w-1/2">
                            <div class="relative">
                            <label for="status" class="block text-sm font-medium text-gray-700">Current Status :  {{($singleJob->status)?'Active':'Inactive'}}</label>
